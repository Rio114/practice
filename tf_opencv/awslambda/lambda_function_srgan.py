...
MODEL_PATH = '20200522_generator.h5'
...
def lambda_handler(event, context):
    for record in event['Records']:
        ...
        tmpkey = key.replace('/', '')
        print('before calling uuid')
        download_path = '/tmp/{rand}{key}'.format(rand=uuid.uuid4(), key=tmpkey)
        upload_path = '/tmp/resized-{key}'.format(key=tmpkey)
