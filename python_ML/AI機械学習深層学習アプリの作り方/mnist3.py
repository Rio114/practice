...
with tf.name_scope("conv2"):
    w_conv2 = tf.Variable(tf.truncated_normal([FILTER_SIZE, FILTER_SIZE, FILTER_NUM, FILTER_NUM], stddev=0.1), name='w_conv2')
    b_conv2 = tf.Variable(tf.truncated_normal([FILTER_NUM], stddev=0.1), name='b_conv2')

conv2 = tf.nn.conv2d(pool1, w_conv2, strides=[1, 1, 1, 1], padding='SAME') + b_conv2
pool2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

with tf.name_scope("dense"):
    w_dense = tf.Variable(tf.truncated_normal([7 * 7 * FILTER_NUM, ALL_CON], stddev=0.1), name='w_dense')
    b_dense = tf.Variable(tf.truncated_normal([ALL_CON], stddev=0.1), name='b_dense')

pool2_flat = tf.reshape(pool2, [-1, 7 * 7 * FILTER_NUM])

dense = tf.nn.relu(tf.matmul(pool2_flat, w_dense) + b_dense, name='dense')
